...
interface QueryItem {
  ...
  subscription_id: string; expires_at: string; created_at: string
}
interface ResponseMedia { media_type: string; url: string; caption: string | null }
...
// Per-subscription Queries page (Option A, 2026-06-18). Mirror of
// /my-queries but scoped to one subscription via the new
// ?subscription_id filter on /farmer/queries. Reached from the
// Crop Detail "Queries" tile.
export default function SubscriptionQueriesPage() {
  ...
  const [hasPrimaryExpert, setHasPrimaryExpert] = useState<boolean>(true)

  useEffect(() => {
    if (!getToken()) { router.replace('/register'); return }
    api.get<QueryItem[]>(`/farmer/queries?subscription_id=${subscriptionId}`)
      .then(r => setQueries(r.data))
      .finally(() => setLoading(false))
    // Re-read the sub to decide whether the "+ Ask Expert" button
    // should be enabled. Mirrors the same gate as the Crop Detail tile.
    api.get<Array<{ id: string; client_has_primary_expert: boolean }>>('/farmer/my-subscriptions')
      .then(r => {
        const found = r.data.find(s => s.id === subscriptionId)
        if (found) setHasPrimaryExpert(found.client_has_primary_expert)
      })
  ...
  // Single QueryCard used by all three buckets so the expand-to-detail
  // behaviour is identical regardless of status. User correction
  // 2026-06-18: farmer must see "what I asked" + "what came back"
  // side-by-side on every query, otherwise the response feels
  ...
  function QueryCard({ q, variant }: { q: QueryItem; variant: 'pending' | 'responded' | 'closed' }) {
    ...
    const borderClass =
      variant === 'responded' ? 'border-green-100' : 'border-[#DDD0B8]'
    ...
    <div className="min-h-screen bg-[#F5F0E8]">
      ...
      <div className="pb-20 px-4 max-w-lg mx-auto">
        ...
        <button
          ...
          className="w-full mb-4 py-3 rounded-2xl text-white text-sm font-semibold bg-[#3A7D44] disabled:opacity-50 active:scale-95 transition-transform">
          {t('askExpertCta')}
        </button>
        ...
        {loading ? (
          ...
          <div className="mt-4 text-center py-16">
            ...
            <p className="text-[#6B3F1F] font-medium mt-3">{t('emptySub')}</p>
            <p className="text-[#7A8C7E] text-sm mt-1">{t('emptySubHint')}</p>
          </div>
        ) : (

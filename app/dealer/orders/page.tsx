...
function DealerOrdersInner() {
  ...
  return (
    <div className="min-h-screen bg-[#F5F0E8]">
      ...
      <div className="pt-16 pb-20">
        ...
        <div className="px-4 pt-3 relative">
          ...
          <button onClick={() => router.push('/dealer/history')}
            className="absolute top-[53px] right-4 text-xs font-semibold text-[#7A8C7E] active:text-[#1E40AF]">
            {t('historyChip')} →
          </button>
        </div>
      ...
      <BottomNav color="#1E40AF" activeRole="DEALER" />
    </div>
  )
}
  ...
  return (
    <div className="px-4 py-3 bg-[#F5F0E8]/40">
      ...
      <div className="flex items-center justify-between gap-2 mb-2">
        <p className="font-mono text-xs font-semibold text-[#1E40AF] tracking-wide truncate">
          {orderId}
        </p>
        {head.is_seed ? (
          <span className="text-[10px] font-semibold text-blue-700 bg-blue-50 px-1.5 py-0.5 rounded-full shrink-0">
            {t('seedTag')}
          </span>
        ) : (
          head.category && (
      ...
      <div className="flex items-start gap-3">
        ...
        {head.farmer_phone && (
          <a href={`tel:${head.farmer_phone}`}
            ...
            className="shrink-0 inline-flex items-center gap-1.5 px-3 py-2 rounded-xl bg-blue-100 text-blue-800 text-xs font-semibold">
        ...
        <div className="flex items-start gap-2 mt-2 pt-2 border-t border-[#F0E8D6]">
          ...
          {head.facilitator_phone && (
            <a href={`tel:${head.facilitator_phone}`}
              ...
              className="shrink-0 text-xs bg-blue-100 text-blue-800 px-2.5 py-1.5 rounded-lg font-semibold">
              {t('callBtn')}
            </a>
          )}
        </div>
        ...
        <>
          ...
          <div className="flex gap-2">
            <button onClick={onAccept} disabled={busy}
              className="flex-1 bg-blue-600 disabled:opacity-50 text-white text-xs font-semibold py-2.5 rounded-xl">
              {t('acceptCta')}
            </button>
...
function PackingChunk({
  ...
  return (
    <div>
      {order.packing_code && (
        <div className="px-4 py-2 bg-blue-600 text-white flex items-baseline justify-between">
        ...
        <div className="flex items-center gap-2 flex-wrap">
          {order.farmer_phone && (
            <a href={`tel:${order.farmer_phone}`}
              onClick={e => e.stopPropagation()}
              className="text-[11px] font-semibold text-[#1E40AF] bg-blue-100 px-2.5 py-1 rounded-lg">
              {t('callFarmer')}
            </a>
          )}
      ...
      <div className="divide-y divide-blue-100">
        {order.packing_items.map(it => (
          <div key={it.id} className="p-3 flex items-baseline justify-between gap-3">
            ...
            {it.price != null && (
              <p className="text-sm font-bold text-[#1E40AF] shrink-0">₹{it.price.toLocaleString(locale)}</p>
            )}
          </div>
        ))}
      </div>
      <div className="px-3 py-2 bg-blue-50/40 border-t border-blue-100 flex items-center justify-between">
        <p className="text-[11px] text-[#7A8C7E]">{t('totalLabel')}</p>
        <p className="text-sm font-bold text-[#1E40AF]">₹{total.toLocaleString(locale)}</p>
      </div>
      <div className="p-3 flex gap-2">
        <button onClick={onShare} disabled={busy}
          className="flex-1 bg-[#1E40AF] text-white text-xs font-semibold py-2.5 rounded-xl disabled:opacity-60">

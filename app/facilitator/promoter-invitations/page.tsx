...
export default function FacilitatorPromoterInvitationsPage() {
  const router = useRouter()
  const t = useTranslations('facilitator.promoterInvitations')
  ...
  const load = useCallback(async () => {
    setLoading(true)
    try {
      const { data } = await api.get<PromoterInvitation[]>('/facilitator/promoter-invitations')
      setInvitations(data)
    } finally {
      setLoading(false)
    }
  ...
  async function confirm() {
    ...
    try {
      const endpoint = pending.action === 'accept'
        ? `/facilitator/promoter-invitations/${pending.id}/accept`
        : `/facilitator/promoter-invitations/${pending.id}/decline`

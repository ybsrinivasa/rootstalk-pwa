      ...
      <div className="pt-16 pb-20 px-4 max-w-lg mx-auto">
        {loading ? (
          ...
          <div className="mt-4">
            ...
            {ongoing.length > 0 && (
              <section>
                <p className="text-xs font-semibold text-stone-400 uppercase tracking-widest mb-3">Active Subscriptions</p>
            ...
            {completed.length > 0 && (
              <section>
                <p className="text-xs font-semibold text-stone-400 uppercase tracking-widest mb-3 mt-6">Past Seasons</p>
                <div className="space-y-3">
                  {completed.map(sub => (
                    <SubCard
                      key={sub.id}
      ...
      {qrSub && (
        <div
          ...
          <div
            ...
            <p className="text-center font-semibold text-stone-800 mb-4">Crop Record QR</p>
            {loadingQr ? (
              ...
              <div className="flex flex-col items-center gap-4">
                ...
                <p className="text-stone-400 text-xs text-center max-w-xs px-4">
...
function SubCard({
  ...
  return (
    <div className="w-full bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden text-left">
      <button
        ...
        <div className="px-4 py-2 flex items-center justify-between" style={{ background: colour + '18' }}>
          ...
          <span className={`text-xs px-2 py-0.5 rounded-full font-medium ${sub.status === 'ACTIVE' ? 'bg-green-100 text-green-700' : sub.status === 'WAITLISTED' ? 'bg-amber-100 text-amber-700' : 'bg-slate-100 text-slate-500'}`}>
            {sub.status}
          </span>
        </div>
      ...
      <div className="px-4 pb-3">
        <button
          ...
          className="text-xs font-medium px-3 py-1.5 rounded-xl border border-stone-200 text-stone-600 mt-3 flex items-center gap-1.5">

...
export default function SeedVarietiesPage() {
  ...
  if (selected && !confirming) {
    return (
      <div className="min-h-screen bg-slate-50">
        <PWAHeader title={selected.name} activeRole="FARMER" />
        <div className="pt-16 pb-24">
          ...
          <div className="px-4 py-5 space-y-4">
            <div>
              ...
              <p className="text-sm text-slate-500 mt-0.5">{selected.variety_type} · {selected.crop_cosh_id}</p>
            </div>

            {selected.description_points.length > 0 && (
              <div className="bg-white rounded-2xl border border-slate-100 p-4">
                <p className="text-sm font-semibold text-slate-700 mb-2">About this variety</p>
            ...
            {selected.dus_characters && Object.keys(selected.dus_characters).length > 0 && (
              <div>
                <button onClick={() => setShowDUS(!showDUS)}
                  className="w-full flex items-center justify-between bg-white rounded-2xl border border-slate-100 p-4">
                  <span className="text-sm font-semibold text-slate-700">Technical Details (DUS)</span>
                  <span className="text-slate-400">{showDUS ? '▲' : '▼'}</span>
                </button>
                {showDUS && (
                  <div className="bg-white rounded-b-2xl border border-t-0 border-slate-100 px-4 pb-4">
                    <table className="w-full text-xs">
                      <tbody>
                        {Object.entries(selected.dus_characters).map(([k, v]) => (
                          <tr key={k} className="border-b border-slate-50">
                            <td className="py-1.5 text-slate-400 w-1/2">{k}</td>
                            <td className="py-1.5 text-slate-700 font-medium">{v}</td>
                          </tr>
                        ))}
                      </tbody>
                    </table>
          ...
          <div className="fixed bottom-0 left-0 right-0 p-4 bg-white border-t border-slate-100 max-w-lg mx-auto">
  ...
  if (confirming && selected) {
    return (
      <div className="min-h-screen bg-slate-50 flex flex-col items-center justify-center px-6">
        <div className="bg-white rounded-2xl border border-slate-100 p-6 text-center max-w-sm w-full space-y-4">
          <div className="w-16 h-16 bg-green-50 rounded-full flex items-center justify-center mx-auto">
            <span className="text-3xl">🌾</span>
          </div>
          <div>
            <h2 className="font-bold text-slate-900 text-lg">Confirm Selection</h2>
            <p className="text-slate-500 text-sm mt-1">
              You are selecting <strong className="text-slate-800">{selected.name}</strong> for your crop.
            </p>
  ...
  return (
    <div className="min-h-screen bg-slate-50">
      <PWAHeader title="Select a Variety" activeRole="FARMER" />
      <div className="pt-16 pb-24 px-4 max-w-lg mx-auto">
        ...
        {varieties.length === 0 ? (
          <div className="text-center py-20">
            ...
            <p className="text-slate-500 font-medium">No varieties available yet</p>
            <p className="text-xs text-slate-400 mt-1">Your company has not added seed varieties for this crop</p>
          </div>
        ) : (
              ...
              <button key={v.id}
                ...
                <div className="p-4">
                  ...
                  {v.description_points.length > 0 && (
                    <p className="text-xs text-slate-600 mt-2 line-clamp-2">{v.description_points[0]}</p>
                  )}
                </div>
              </button>
            ))}
